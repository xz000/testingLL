...
1. **S021 虔诚 500 回血圈**：可加一个治疗范围环（区别于伤害环）。
2. **冲锋/燃烧接触 AoE**：给 `splash_damage` 输出一个表现事件（如 `CombatEvent::Splash{pos,radius}`），
   客户端用 `Blast` 画圈，避免「看不见的范围伤害」。
3. 若未来 AoE 半径随等级成长：统一改由 `stats.radius` 驱动，并让客户端预览同步。
