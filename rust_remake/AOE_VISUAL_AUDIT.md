...
| 项 | 之前 | 现在 |
|---|---|---|
| `CombatEvent::Explode` 视觉 | `Ring`：`radius*(0.5 → 2.0)`，最大 2× 真实半径 | `Blast`：**固定精确半径**（淡填充 + 2.5px 亮环），淡出 |
...
- 表现：`client/src/fx.rs`（`FxKind::Blast` 固定精确半径；`HealRing` 从中心扩大）、`client/src/main.rs`（`CombatEvent::Explode/PillarBreak`）。
