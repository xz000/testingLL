...
- 设置界面：`↑/↓` 选择行、`←/→` 调值、`回车` 切换/编辑；`Esc`/`Q` 返回（**自动保存**）。
- 鼠标：沿用 `HitRegistry`（点行选中、点值调/编辑、可点返回）+ 底部操作条 + 按键提示。
- 快捷：对局/大厅内 `M` 一键静音。
...
- [ ] **P2-0 占位素材**：`tools/` 脚本合成 `client/assets/audio/*.wav`（A/B 优先）。
- [ ] **P2-1 音频基础设施**：`local_settings.rs`（读写+默认+单测）；`AudioBank`（加载/播放/静音/降级）。
- [ ] **P2-2 主菜单「设置」界面**：第 4 卡片 + 设置界面（行列表 + 键鼠 + 提示）+ 持久化 + `M` 静音。
- [ ] **P2-3 战斗音效**：§2 A01–A13。
- [ ] **P2-4 流程音效**：§2 B01–B06 + `no`/`Vo`/`yx`。
- [ ] **P2-5 播报语音**：§1 事件对齐（文本横幅已有/待补信号：Hattrick/Vampire/Denied/Burnout/…）→ 播 V01–V27（或先占位）。
- [ ] **P2-6 UI/商店音效**：§2 C/D。
- [ ] **P2-7（可选）BGM**。
