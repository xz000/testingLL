...
**已做**：
- 纯本地/可单测：包详情（作者/音效覆盖/BGM 场景）、生成示例包、逐场景试听 BGM、
  「要发布的包」独立选择、发布可见性、复用物品 id。
- **工坊物品一屏概览**（设置页 `创意工坊物品` 行打开）：列出已订阅物品 → 名称 + 状态
  （已就绪 / 需更新（回车下载）/ 下载中 x% / 已订阅等待下载）；`回车`=下载/更新、`O`=覆盖层开工坊页、`R`=刷新。

**有意不做（靠 Steam 覆盖层/客户端覆盖，避免重复造轮子）**：
- **游戏内工坊浏览器**（浏览/搜索/按 tag 筛选/订阅/取消/评论）——`O` 直接用 **Steam 覆盖层网页版**
  （`steamcommunity.com/app/908660/workshop/`）即可。
