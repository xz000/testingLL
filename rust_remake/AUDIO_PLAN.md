...
| 多重击杀 Double/Multi/Mega/Ultra/Monster | `Qx/tx/sx/Tx/Sx` | ⬜ 待实现：098c 的“多重击杀时间窗”（`dn[VI+$C]` + `Wn`） |
| Ludicrous Kill | `Io` | ⬜ 待实现：击杀“0 死”对手 |
| Hattrick / Vampire | `Ux` / `Oo` | ⬜ 待战斗信号（单次命中 ≥3） |
| Silencer / Pancake / Burnout / Denied / Last Second Save | `Eo`/`ao`/`Xo`/`Wx`/`wx` | ⬜ 待战斗信号（P1 扩展共用） |
| 学习 / 升级完成 | `Ro` ResearchComplete | ⬜ 待接（购买/升级成功点） |
...
- [~] **P2-5 播报语音**：已接 首杀 / 连杀 3..10/>10 / 胜利；多重击杀·Hattrick·Vampire·Denied 等需先补信号。
- [~] **P2-6 UI/商店音效**：已接 主菜单/设置导航；商店只接“研究完成”（买/卖 098c 无）。
- [ ] **P2-7（可选）BGM**。
