...
- [ ] **P2-4 流程音效**：**098c 无回合/倒计时/缩圈/出界音** → 不接；开局 `Vo` / 终局过场 `yx` **已接**。
...
`试听当前音效包`（播一个代表性 cue）、`试听 BGM 场景`（循环 跟随场景/menu/lobby/battle/result，临时强制 BGM 场景）、
`生成示例包`（在本地目录生成 `ExamplePack/`：结构+清单+静音占位+README）。
选中包行时，底部显示**包详情**：`作者 … ｜ 音效 N/总数 ｜ BGM：场景列表`。
选中即生效并写回；改包后自动重扫 + 热重载（旧 BGM 淡出）。

> 发布映射 `包id → 物品id` 持久化在 `settings.txt` 的 `published.<id>=<fileid>` 行；上传失败会清除该记录（下次新建）。
...
### 8.8 增强项取舍（2026-09-19）

**已做（均纯本地/可单测）**：包详情（作者/音效覆盖/BGM 场景）、生成示例包、逐场景试听 BGM、
「要发布的包」独立选择、发布可见性、复用物品 id。

**有意不做（靠 Steam 覆盖层/客户端覆盖，避免重复造轮子）**：
- **游戏内工坊浏览器**（浏览/搜索/按 tag 筛选/订阅/取消/评论）——设置页 `浏览创意工坊` 按钮直接用
  **Steam 覆盖层网页版**（`steamcommunity.com/app/908660/workshop/`）即可。
- **游戏内物品删除/管理**——物品页/Steam 客户端可删。
- **订阅/下载状态列表**——下载进度看 Steam 客户端「下载」页；**“是否就绪”由「音效包 / BGM 包」列表自身体现**
  （只有已下载安装的包才会被目录扫描列出）；设置页 `创意工坊物品` 行仍显示 `已订阅 N（就绪 M）` 并一键开工坊页。
