...
## 六、双机实测结论（2026-09-13，带 5s 统计）

样本：约 3000 帧（~55s），host / client 各一份 `logs/app-*`。

**关键数字**：
- **ping 6~8ms（两端）** —— 链路 RTT 极小，不是宽带/延迟问题。
- client `pending_max=1` —— 没有帧积压；**不是“帧没到堆在缓冲”**。
- host `emit=` 分桶：多数 ≤25ms，但每 300 帧有 **~5~25% 落在 33ms**（晚 1 帧）、少量 50/67/100ms；
  client `frame=` 分桶与之相印。
- **sim 实际频率 ~52Hz（47~56Hz 波动）**：300 帧耗时 5.4~6.4s（而非 5.0s）→ **比 60Hz 慢约 13% 且抖**。
- host `wait_ticks` 每 300 帧长 ~300~400（大量 update 迭代花在“等输入”上）。

**结论**：瓶颈不在网络带宽/延迟，而在 **host 产帧与“输入到达 / 本地帧时长”耦合**（即 B 的根源）：
host 每个 tick 都要等到某端的新输入才产帧，于是 sim 频率被拉到与两端 update 节奏相近（~52Hz），
并继承本机帧时长抖动（偶发 66~134ms）。

**可选下一步**（待讨论）：
1. **渲染插值**（不碰网络、不增延迟）：sim 仍 ~52Hz，但画面按 `prev→cur` 插值 → 观感平滑。
2. **host 固定 60Hz 产帧 + 缺输入时“保持连续量”**（held input）：不新增客户端延迟；
   缺输入那一帧用上一条的 `set_target`、丢弃 `cast/queued` 等离散动作（需避免重复施法）。能拉回稳定 60Hz。
3. 保持现状。

## 七、修复后复测（2026-09-13，已实施 held + 插值）

样本：约 1800 帧。host / client 各一份 `logs/app-*`。

| 指标 | 修复前 | 修复后 |
|---|---|---|
| host sim 频率（逐窗口） | 46.9–55.5（均值 ~52.5） | **59.9 / 60.1 / 59.9 / 59.9 / 60.1** |
| host `emit` 33ms 占比 | 5~25% | **0~3%（多数窗口为 0）** |
| host `[jit]` | 182 / 3000（~6%） | **4 / 1800（~0.2%）** |
| `waiting for client input` | 72 次 | **0 次** |
| client `[jit]` | 234 / 3000 | **46 / 1800（~2.5%，全为本机/投递 33/50ms）** |
| client `pending_max` | 1 | 2–3（小，会追上） |

**结论**：
- **held-continuous 固定节拍**把 host 从“被输入到达牵着走”拉回**稳定 60Hz**，并消除了“等输入停摆”。
- **渲染插值**包下了剩余的 client 侧 33/50ms 帧到达抖动 → 肉眼流畅（用户确认）。
- 残余 client `[jit]` ~2.5% 为**网络投递/排队**，host 端管不到；已被插值抹平，当前不需再处理。

余项（低优先，按需）：弹体也做插值；`[stat]` 里的 `wait_ticks` 在固定节拍后恒不再增长（可改/可删）。
