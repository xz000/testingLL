...
2. **host 固定节拍产帧 + 输入延迟缓冲**（消除 B，**未实施**，需联机验证）
   ...
   - **详细设计草案见 `FRAME_SYNC_INPUT_DELAY_DESIGN.md`**（协议方案 A/B、数据结构、伪代码、测试、风险）。
3. **client 收敛追赶**（**已实施**，见第四节）
   - `accumulator` 夹到 `MAX_CATCHUP_STEPS=4` 步，避免一帧快进 10+ 步；
   - 输入改为**每次 `update` 至多一条**（移出追赶循环），避免输入突发；
...
- Steam client / 局域网 client：输入改为**每次 `update` 至多上行一条**（移出追赶循环），避免抖动时输入突发。
- （渲染插值仍未做。）
