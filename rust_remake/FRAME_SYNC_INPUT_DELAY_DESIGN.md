...
> 状态：**搁置（用户裁定 2026-09-13）**。
>
> **决策**：走**经典 RTS 式 lockstep**（“等齐再推进”，延迟来自网络本身），**不引入固定 D 帧输入延迟**
> （那是格斗/delay-based 派）。先由用户实测当前版本手感，再决定后续方向。本文保留作为备选方案库。
>
> 若实测后确实需要改善，优先级顺序（均不动世界/协议语义的前提下）：
> 1. **渲染插值**（不加输入延迟，只平滑画面）——最符合 RTS 手感，推荐先试；
> 2. 降 tick 到 30Hz（用 RTS 的粗粒度吸收抖动；代价：手感/带宽/常量）——需慎重；
> 3. 固定 D（本文方案）——仅在以上不可接受时重新考虑；
> 4. 客户端预测/回滚（Dota2/LOL 路线）——工程量最大。
