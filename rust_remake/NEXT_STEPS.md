...
> **进度**：✅ 阶段 1（掉线处理+重连）✅ 阶段 2（快照广播）✅ 阶段 3（主机迁移）全部已落（2026-08-25）且真机复验通过，
...
## █ 098c 解析 + 校准 3 项高优先级差异（2026-09-08）
> 详见 `SKILL_AUDIT_098b_vs_rust.md` **§8**（凡与旧推断冲突，以 §8 为准）。
- **098c 素材已完整解密，无需重新破解**：`098c/` = `docs/MECHANICS.md`（完整机制）、`docs/GGEPSEUDO.md`、
  `data/spells.json`（26 槽数值）、`data/items.json`、`out/war3map_pretty.j`（364KB 去混淆脚本）、`out/w3a_strings.txt`。
- **⚠ 关键结论**：098c 实际名册 26 槽**不含电弧/冲击/爆炎/操纵**——这四个是 **098b 独有技能**，098c 里根本没有，
  因此**无法靠解析 098c 实现**；`port_098b/` 与 CE 1.10B 亦未检索到其实现级参考，只能按 tooltip 保守建模
  （`S023 电弧` 即属此类，**去留待定**）。098c 名册**含** `S022 Mirror`。
- **已按 098c 校准（审计 §5.3 三项）**：
  - **#1 `S017B 禁锢·沉默`**：冷却 25 → **恒定 16.0**（098c `silence_cd 16.0`，与 A 形态 `cooldown 25` **并列**，
    证明 B 有独立 CD）。**推翻 §7.3 原「非 bug，不改」结论**（该结论当年是据代码注释推断，现已有硬数据）。
  - **#4 `S015 火焰喷射`**：单发 2.4+0.2L → **2.6+0.0632L**（camp2 对齐至 L20=3.8；初版误用 0.3 使 L20≈8.3 超 098c 上限 3.8，已修）；
    连发数硬编码 8 → **随等级 6→12**（走 `growth.extra_*` → `stats.extra`）；冷却 L20 7.0 → **10**（098c `[16,10]` 下限）。
  - **#5 `S016 弹跳球`**：⚠️ 初版据 `spells.json` 标量 `cooldown 20` **误判为恒定、结论"Rust 正确"有误**。
    完整工具提示（`w3a_strings.txt` Bouncer）确证 CD **20→13（-1/级，8 档）** → Rust 原恒定 20 **系错误**，已加 **`cooldown_delta -0.3684`（L20=13）** 修正；
    伤害 `6+1.0L` 与 detailed `gX=5+L` 及 098c 每级 +1 吻合，维持不变。
- **测试**：`s009_s014_s015_s016_match_spec` 里 S015 的旧断言（L20 CD≈7 / 伤害 2.4 / 连发 8）锁定的是 098b 文档值，
  已按 098c 更新为（L20 CD≈10 / 伤害 2.6 / L1 连发 6 / L20 连发 12）。202 全绿，clippy 默认+steam 全绿。

## █ 用 098c 完整工具提示校准 S002/S008/S009/S010（2026-09-08）
> 数据源：`098c/out/w3a_strings.txt`（逐档完整工具提示，**权威**）；`data/spells.json` 仅采样（标量/成对），不可用于判定是否恒定。
> 校准准则：20 级技能用 **camp2 端点对齐**（`Rust L20 ≈ 098c Lmax`，`delta=(top-base)/19`）；`max_level` 已按 098c 档数设定（S002=9）。
- **S002 闪电**：无改动。原 `damage 7+1.0L`、`CD 16.5-0.5625L` 与 098c 9 档（7→15 / 16.5→12）完全吻合。
- **S008 陨石**：无改动。CD 已 20→16.5（camp2）；伤害沿用 detailed `gX=10+2L`（L1=12 落在 098c 显示区间 7–14 内）。
  **距离衰减区间（7–14→14–28）Rust 未建模**，记为已知简化。
- **S009 分裂弹·目标**：`damage_delta 0.0`→**`0.1842`**（L20=6.5，098c 每弹 3.0→6.5）。
- **S010 疾风步·冲锋**：`damage 4.6+0.8L`→**`5.4+0.2947L`**（L20=11，098c 背刺 5.4→11）；CD/dur 原已正确。
- 同步**修正上一轮的 S015/S016 误判**：S015 单发 `delta 0.3`→**`0.0632`**（L20=3.8）；S016 恒定 CD 20→**`cooldown_delta -0.3684`（L20=13）**。
- **测试**：`s009_s014_s015_s016_match_spec`、`s010_form_a_charge_vs_b_invisibility` 断言同步更新；202 全绿，clippy 默认+steam 全绿。
- **待清理 → 已闭环（见下一节）**：`S008b` 岩浆、以及「S011–S020 的 20 级外推与 098c 是否逐一对齐」两项，已在本轮 S011–S020 对齐中一并处理。

## █ 核查 + camp2 对齐 S011–S020（A + B 形态，2026-09-09）
> 数据源同上（`098c/out/w3a_strings.txt` 权威逐档表）。**关键认知**：098c 进度常**非严格线性**（如 Thrust CD 16.5→14.5→12→11→10→9→8→7），Rust 只有线性 `base+delta×(L-1)`；camp2 只保证**端点吻合**（Rust L20 ≈ 098c Lmax），中段阶梯被压缩。
> `max_level` 映射：S011=9（精确匹配）、S020/S021/S001=4（精确）、其余 S012–S019=20（camp2）。
- **核查结论**：S011 已一致；S015/S016 前轮已校准；**S012/S013/S014/S017/S018/S019/S020 的 A 形态普遍偏离**（多为旧「spec 自定斜率」外推过头，如 S019 CD L20=16 vs 098c 上限 8），全部按 098c camp2 重写（明细表见 `SKILL_AUDIT_098b_vs_rust.md` §5.6）。
- **A 形态改动**：S012（CD −0.5 / 伤害 0.1684 / 射程 700+21.05）；S013（CD −0.5263，L20=6）；S014（CD −0.2895 / 伤害 6+0.3684 / 持续 4+0.3684）；S017（CD base 25→**16**、delta −0.1842，L20=12.5）；S018（CD 26→**25** 恒定、伤害 0.3/0.0737）；S019（CD 17/−0.4737 → L20=8、伤害 0.2/0.0842）；S020（伤害 12+4L → **11/0.6667**，098c 为 11/12/13）。
- **B 形态全部对齐**：S008B / S009B / S010B / S012B / S013B / S014B / S015B / S016B / S017B / S018B / S019B（明细见审计文档 §5.6）。
- **⚠ 实现坑（下次注意）**：`stats.max_distance` 与 effect 里硬编码的 `max_distance` 是 `max()` 关系（后者是**下限**），改射程必须**两处同步**，否则 growth 改了不生效（S012 770→700、S013B 600→900 已同步）。
- **测试**：`s011_s012_s013_match_spec`、`s017_s018_s019_match_spec`、`s001_s020_s021_match_spec`、`s009_s014_s015_s016_match_spec`，以及 `world.rs` 的 `doc_s019_s018_growth_matches_doc`、`roles_replace_f_slot_and_apply_buffs` 断言同步更新；门禁 build / test(202+34+9+10) / clippy 默认+steam **全绿**。
- **语义遗留（非数值，待 M2 决策）**：① S017 A 建模为「缠绕/残废」而 098c `Disable` 实为「沉默 5s 恒定」（本轮**只对齐了 CD**，行为建模未改）；② S018 `pull_speed`（吸引力）仍未随 098c Force 12→19 缩放（TODO）；③ S012B 凤凰为 Rust 自定升级体（098c 无凤凰表），仅 CD 借 Thrust camp2，伤害 4+0.5L 为自定。
...
2. **主菜单/流程 UI“乱”**（进行中，2026-08-17 起系统性重做）：用户多次抱怨菜单乱、键太多；已把就绪/配好拆成 U/P，现按「UI_REDESIGN」分增量重做所有菜单排版。

### 🔧 UI 重做进行中（2026-08-17 起，分增量）——设计定稿 + S1 已落
**目标**：主菜单及各阶段界面系统性做清爽。局域网进房（S3）此时**不做**，局域网入口保留“建设中/需命令行”提示。
**已核实 steamworks 0.13 能力（据此设计，不越界）**：
- `create_lobby(max_members≤250)`：建房时设人数上限（S2 界面开放 2~64；steamworks 无**开房后改人数**接口——`lobby_member_limit` 只能读，`SetLobbyMemberLimit` 未暴露）。
- `set_lobby_data`/`lobby_data`（key≤255B、value≤8192B）：房间名/备注等元数据，**开房后可随时改**（编辑房间信息用）。
- `lobby_owner`/`lobby_member_count`/`lobby_member_limit`：房间列表显示“房主·人数 x/M”（加入前可读）。
- `lobby_members`/`Friends::get_friend(id).name()`：昵称。`set_lobby_joinable(bool)`：开房后**锁定/解锁房间**（他人能否再加入）。
**设计定稿（用户确认）**：
- 建房设置界面用**键盘文本输入框**（Steam 卡自由输入）；房间名默认「{昵称}的房间」；人数 2~64，`+/-` 步进 + **数字直接输入**；房间名/备注可编辑。
- 人数上限建房时固定（steamworks 限制），不加“开房后改人数”，用「房间锁」代替（L 锁定/解锁）。
- 「加入房间」改为**房间列表浏览并加入**（去掉测试用的自动加入）：列表显示房主·房名·人数·备注，方向键选中+回车。
**一个未满足项（已确认，需后续实现，不在纯 UI 范围内）**：
- 用户提“人不满但全员准备就绪也要能启动”。
...
- **冗余清理**：去掉已实际使用的 `STEAM_MAX_PLAYERS`、`steam_create_players`、`steam_roster`、`steam_all_ready` 上的多余 `#[allow(dead_code)]`（它们现在都被读写）；`BOTS` 常量保留（设计上留作将来“带 AI 测试”模式）；删除 session.rs 未使用的 `ROOM_MAX_KEY`（早期“建房写上限”方案的残留，已改用“对局开始定参与者数”）。
