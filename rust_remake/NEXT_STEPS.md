...
> **进度**：✅ 阶段 1（掉线处理+重连）✅ 阶段 2（快照广播）✅ 阶段 3（主机迁移）全部已落（2026-08-25），
> workspace 124 全绿，build/test/clippy（默认+steam）全绿。阶段 1/2 已真机复验；**阶段 3（主机迁移）待真机复验**：
> Steam 对局中 host 直接退出 → 其余端自动选新 host 接管、对局不断。
> 阶段 3 完成后接 Steamworks 增强：好友邀请 + Rich Presence → 成就/排行榜/头像/Ping → 云存档。
