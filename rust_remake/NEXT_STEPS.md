...
- **已按 098c 校准（审计 §5.3 三项）**：
  ...
  - **#4 `S015 火焰喷射`**：单发 2.4+0.2L → **2.6+0.3L**；连发数硬编码 8 → **随等级 6→12**
    （走 `growth.extra_*` → `stats.extra`，由 `SkillEffect::Sweep` 执行处读取）；冷却 L20 7.0 → **10**（098c `[16,10]` 下限）。
  - **#5 `S016 弹跳球`**：098c `cooldown 20`（标量=恒定）+ `damage 6` → **Rust 现值本就正确**，无代码改动，仅闭环审计。
