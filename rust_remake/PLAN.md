...
> - 单测 53 全绿（含护盾反弹、疾跑返半回血、幻象两段式、影身召回、障碍推出、二段闪、冲刺斩解除、闪墙、滚动火球 DoT、撒弹线爆散、连推重踢、回旋镖、香蕉弹、导弹）。
> **待做**：技能成长数值手感调优；冷却 HUD 换图标贴图（阶段 4 美术）；`shift` 指令队列（阶段 2 待做第 5 项）；其余树（T/Y/F/G）技能落地。
