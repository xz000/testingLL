...
> - **Y 树全部落地（复刻原版）**：Y1/Y1b 回拉线 `Tether`（场效应 `pull` 拉向施法者 + DoT，Y1b 沿线段扫射）；Y2 撞击迟缓 `Bullet`（命中伤+击退）；Y2b 束缚线 `BindLine`（线段束缚 Tied 禁施法）；Y3 引力场 `Gravity`（场效应吸附）；Y3b 星域 `Star`（范围敌 DoT + 施法者回血）。此时 `step_area_forces`（原先空的场效应钩子）正式接入。
...
> - 单测 61 全绿（含护盾反弹、疾跑返半回血、幻象两段式、影身召回、障碍推出、二段闪、冲刺斩解除、闪墙、滚动火球 DoT、撒弹线爆散、连推重踢、回旋镖、香蕉弹、导弹、吸血链、跳弹、扇扫/扇射、蓄力跳弹、回拉线、撞击迟缓、束缚线、星域）。
> **待做**：技能成长数值手感调优；冷却 HUD 换图标贴图（阶段 4 美术）；`shift` 指令队列（阶段 2 待做第 5 项）；其余树（F/G）技能落地。
