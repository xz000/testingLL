...
在学习阶段从该树的可选技能里花钱购买一个（同树互斥、整场锁定），再花钱升级；无洗点/解绑。
...
2. **键位绑定 + 技能购买（已完成）**：`PlayerProfile` 增加 `key_slots[8]`（每个键绑定哪个技能）+ `gold_spent`；
   提供 `purchase_skill`（花钱购买 1 级，同树互斥、整场锁定、买越多越贵，对标 098c）+ `upgrade_skill`（升级已购技能）。
   已移除 `unbind_skill` / `respec`（全额退款洗点）。客户端学习阶段：按字母键选中树 → 数字键/点击花钱购买该树技能
   → `=` 升级该键已购技能 → `B` 切形态。开局默认每个键首技能树待购买（不预绑）。
