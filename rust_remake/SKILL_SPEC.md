...
| 槽 | 名称 | 原版机制 | 当前 Rust |
|----|------|----------|-----------|
| T1b | TLeech 吸血链镖 | 镖命中敌人→吸血回己，并**自动跳链最近下一个**（链式吸血） | ❌ 未实现 |
| T2 | T2Shot 扇扫连射 | 朝目标方向 **~8 发按角度微转转扫**（每 0.1s 一发自爆炸弹） | ❌ 未实现 |
| T2b | T2Volley 扇面齐射 | 同时喷 **4 发扇形**（-30°~+30°）爆炸弹 | ❌ 未实现 |
| T3 | T3Fast 跳弹·衰减 | 高速(20)镖，命中→**跳跃到最近下一个**，伤害逐跳衰减(×1,×0.8,×0.6...) | ❌ 未实现 |
| T3b | T3Fast2 跳弹·蓄力 | 命中→爆炸推+伤，并**原地留返回弹**；累计命中的越多，下次 `damageplus` 越高(+0.3)；失误则归零 | ❌ 未实现 |
| TestLeech | 转镖吸血 | 直射镖，飞行中段**折向最近敌人**，命中吸血回己 | ❌ 未实现 |

### Y 控场树
| 槽 | 名称 | 原版机制 | 当前 Rust |
|----|------|----------|-----------|
| Y1 | Y1BlueLine 蓝线回拉 | 对点击处目标上**蓝线**：持续拉向施法者 + 持续掉血（maxtime） | ❌ 未实现 |
| Y1b | Y1BlueLine2 红线回拉+扇伤 | 同回拉，但红线**沿路径持续射线扫射伤害**所有经过的敌人 | ❌ 未实现 |
| Y2 | Y2Delay 撞击迟缓推 | 直线弹，命中→**推离 + pushtime=2s** | ❌ 未实现 |
| Y2b | Y2Suite 静默束缚 | 施法者身后两点反向对称发射、**收拢交汇成线**，线扫过的人被**束缚 3s（禁施法）** | ❌ 未实现 |
| Y3 | Y3Zone 引力场 | 打出一个飞行场，**持续把附近敌人吸向场中心**+倒计时 4s | ❌ 未实现 |
| Y3b | Y3Zone2 星域持续伤 | 目标点放一颗**星**：范围内敌持续掉血、对自己回血，持续 4s | ❌ 未实现 |
