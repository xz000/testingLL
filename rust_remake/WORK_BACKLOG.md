...
- **解析 098c 并校准 3 项高优先级差异**（2026-09-08，详见审计 §8）：`098c/` 已完整解密（`docs/MECHANICS.md`、`data/spells.json` 26 槽、`out/war3map_pretty.j`），可直接当权威源。
  ⚠ **重要结论**：098c 名册 26 槽**不含电弧/冲击/爆炎/操纵**（098b 独有，098c 没有）→ 这四个**无法靠解析 098c 实现**，`S023 电弧` 去留待定。
  已校准：`S017B 沉默` CD 25→**恒定 16.0**（098c `silence_cd`，推翻 §7.3 旧结论）；`S015 火焰喷射` 单发 2.4→**2.6+0.3L**、连发数 8→**随等级 6→12**、CD 下限 7→**10**；`S016 弹跳球` 098c 证实 Rust 现值正确，**无改码**。
...
| ~~**击退模型 Impulse**（瞬时初速度+逐帧减速，替代定速推）~~ | ✅ **已落（D8 衰减模型）**：`player.rs:359` `push_knockback()` —— 初速按 `effective_kb_reduction()` 缩放、每帧 ×0.9776 衰减至自然停止（`decay:true`）；098b 名册 KI 击退已全走此路径。⚠ 遗留：Unity 版旧技能仍走恒速 `push()`（`player.rs:348`） | `PLAN.md`「待议/搁置」 |
...
## 7. UI 打磨
- **Dota2 式相机平移（可拖动视角）** —— 用户提出，2026-09-10 记录，**暂不实现（记待办）**：
  - 期望交互：① 按住鼠标某键（中键/右键）拖动平移；② 鼠标移到屏幕边缘时朝该方向持续平移（edge pan）。
  - ⚠ **明确不做「自动跟随」**（用户 2026-09-10 定）：即使将来场地变大，也**只做手动平移**，相机不自动跟随本地玩家。
  - **两个定位快捷键**（用户 2026-09-10 补，均为**一次性跳转**，不持续跟随）：
    - ① 回到**场地中心**；② 回到**自己角色当前位置**。
    - 🔑 **具体键位：待定**（用户 2026-09-10 指示「先不着急定键位」）。
    - 键位可用性备忘（届时再挑）：战斗阶段 `Named` 键只处理了 `Enter`/`Escape`；`Tab`/`F1-F3` 仅在**学习期**的
      `poll_learning`（`main.rs:1081-1091`）中使用 → 战斗期 `Space`/`Home`/`F1`/`F2`/`F3`/`Tab` **全部空闲**。
      字母键在战斗期已被技能（C/D/E/R/T/Y/F/G）与 S（停止）占用，故定位键宜用 **Named 键**避免冲突。
  - **现状**：`client/src/main.rs::update_camera`（982）为**固定居中**相机——`scale = min(sw,sh)*0.45 / START_RADIUS`、
    `offset = 屏幕中心`；全场直径 1152 → 约占短边 **90%**，**整场一屏可见**，故当前无需平移；无缩放、无滚轮事件。
  - **触发条件（关键依赖）**：若将来把场地尺度对齐 **098c（约 ±4096 = 8192 宽，约为我们半径 576 的 7 倍）**，
    整场将装不进一屏 → 届时**必须**做相机平移（否则看不见远处对手与缩圈）。只做手动拖动/边缘平移，
    **不自动跟随**；是否另配小地图另议。
  - 实现要点（届时）：在 `update_camera` 里把 `offset` 改为可变（当前恒为屏幕中心），新增 drag 状态与 edge-pan 判定；
    注意平移后 `screen_to_world`（992）自动跟随（已用 `offset/scale`），但学习界面/菜单等屏幕空间 UI 不受影响，需确认不串扰。
- **击杀 / 连杀 / First Blood 上屏播报**（2026-09-10 记录，**暂缓，先只记录**）：
  - 现状：逻辑与文案**都已实现**但**只在控制台**：`main.rs:1354` First Blood、`1363`/`3660` 连杀，
    均为 `eprintln!`；`MatchState::streak_label()`（`meta.rs:501-513`）已实现却**从未用于绘制** → 玩家零反馈。
  - 做法（届时）：复用现有 `steam_toast`（`main.rs:2303-2311`）的样式加一个通用 toast 队列，
    把 `settle_round` 的击杀事件与 `streak_label()` 文案推入队列渲染。数据现成，纯渲染增量。
...
6. ~~**学习阶段交互（暂缓）**：选技能树/绑定仍只用字母+数字快捷键，可加鼠标/方向键~~ ✅ **鼠标已支持（U2）**：
   `client/src/main.rs:231` `learn_hitboxes`（绘制时注册矩形）+ `:1012` `learn_dispatch_click`（update 派发），
   已覆盖 `Page/Tree/Skill/Form/Item/Mastery` 六类操作，与键盘等价。剩「方向键导航」未做（低优先）。
