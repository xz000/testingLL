...
/// 目前使用者均在大厅（Steam）界面，故非 steam 构建下允许未用；接入更多界面后可移除。
#[cfg_attr(not(feature = "steam"), allow(dead_code))]
mod layout;
...
struct Game {
    ...
    /// 学习阶段当前选中的技能下标（树内第 i 个）；点击技能名称=选中查看详情，不直接购买。
    learn_skill_index: Option<usize>,
    /// 商店页当前选中项（ItemId；None=未选）。单击只选中看详情，`=`/回车 才购买/卖出。
    learn_shop_sel: Option<game_core::item::ItemId>,
    /// 成长页当前选中（0..3=精通类；None=未选）。单击只选中，`=`/回车 才购买。
    /// （技能上限突破已移到「技能」页详情的乔丹之石按钮，不再占成长页的一行。）
    learn_growth_sel: Option<usize>,
...
impl Game {
    fn new(ctx: &mut Context, app: AppState) -> GameResult<Self> {
        ...
        Ok(Game {
            ...
            learn_skill_index: None,
            learn_shop_sel: None,
            learn_growth_sel: None,
            learn_page: 0,
            shop_category: 0,
            shop_scroll: 0,
    ...
    /// - 按数字键 1..N → 选中树里第 N 个技能查看详情（不直接购买，与点击行为一致）
    /// - 按 `=` → 购买/升级当前选中的技能（第一次是购买，之后是升级）
    /// - 按 `B` → 切换当前选中技能的形态
    ...
    /// 未购买 → 花钱购买（置 1 级）；已购买 → 升级一级。即"第一次升级就是购买"。
    ...
    fn learn_dispatch_click(&mut self, ctx: &Context) {
        ...
        for action in hits {
            match action {
                ...
                LearnAction::Skill(i) => {
                    // 点击技能名称 = 选中查看详情（不直接购买）：先看描述/二形态再决定买不买
                    self.learn_skill_index = Some(i);
                }
                ...
                LearnAction::Category(cat) => {
                    ...
                    self.shop_scroll = 0;
                    // 清掉选中：换分类后原选中行可能已不在列表里，留着会让「确认」作用到看不见的行。
                    self.learn_shop_sel = None;
                }
    ...
    fn poll_learning(&mut self, ctx: &Context) {
        ...
        // J/K/L：直达 技能 / 商店 / 成长 三页。
        // 选它们是因为：① 三键相邻好按；② 在本界面内全部空闲（`J` 只在主菜单、`L` 只在房间编辑）；
        // ③ 不与施法键 c/r/e/d/y/t/f/g（本页用来选树）冲突。F1-F3 保留为等价别名。
        for (lp, ch) in [(0u8, "j"), (1u8, "k"), (2u8, "l")] {
            if Self::char_just(ctx, ch) {
                self.learn_page = lp;
            }
        }

        // F1/F2/F3：同上的把别名（习惯用功能键的玩家也能直达）。
        for (lp, named) in [
        ...
        // 数字键：选中树里的第 N 个技能查看详情（不直接购买，与鼠标点击行为一致）
        if let Some(key) = learn_key {
            for (i, skill) in key.tree().skills_in_tree().iter().enumerate() {
                ...
                {
                    self.learn_skill_index = Some(i);
                    eprintln!("[learn] select tree={} digit='{}' -> {}", key.letter(), digit, game_core::skill::DefTable::def(*skill).name);
                }
            }
        }
    ...
    fn poll_growth_buy(&mut self, ctx: &Context) {
        // 成长页：数字 1-4 **只选中**（看详情）；`=`/回车 才确认购买（与技能页一致）。
        // 注：这里**不接** `U` —— `U` 是大厅的「就绪」键（见 draw_steam_lobby 一带），
        // 曾在注释里被误写成成长页可用，属于「注释承诺 ≠ 实际绑定」。
        ...
        for (i, _) in MASTERY.iter().enumerate() {
    ...
    fn shop_rows(
        ...
        category: u8,
    ) -> Vec<(String, Option<LearnAction>, &'static str)> {
        let mut rows: Vec<(String, Option<LearnAction>, &'static str)> = Vec::new();
        // ── 可购买：每家族一条（家族式进化，不可跳档；满级则该家族无动作）
        ...
        // ── 已持有：逐件可卖（回收价即 098c `ED` 真值，见 item.rs）
        for &id in me.items.iter() {
            let d = id.def();
            rows.push((
                format!("[卖] {}  +{}G", d.name, d.sell),
                Some(LearnAction::Sell(id)),
                d.desc,
            ));
        }
        rows
    ...
    /// 商店「确认」：购买/卖出当前选中项（键盘 `=`/回车 与点击按钮共用）。
    /// 买卖由**该物品是否已持有**决定（两区的目标 id 不会重叠）。
    ...
    fn poll_shop(&mut self, ctx: &Context) {
        ...
        // 098c `bD` 实证：家族式进化，不允许跳档。
        let rows = match self.meta.profiles.iter().find(|p| p.player_id == me) {
            Some(pr) => Self::shop_rows(pr, self.shop_category),
            None => return,
        };
        ...
        for (i, k) in keys.iter().enumerate() {
            ...
            let Some((_, action, _)) = rows.get(self.shop_scroll + i) else {
                continue;
            };
            if let Some(LearnAction::Item(id) | LearnAction::Sell(id)) = action {
                self.learn_shop_sel = Some(*id);
            }
        }
    ...
    fn draw_scene(&mut self, ctx: &mut Context) -> GameResult {
        ...
        for (la, lb, _) in self.world.lightning_visual.clone() {
    ...
    fn draw_steam_ready_overlay(&mut self, canvas: &mut Canvas, ctx: &Context) -> GameResult {
        ...
        if self.steam_manual_countdown {
            ...
            draw_text(canvas, ctx, hint, 26.0, layout::text_dim(), Point2 { x: cx, y: flow_y + 48.0 }, true)?;
        } else {
        ...
        match self.meta.phase {
            ...
            MatchPhase::Learning => {
                ...
                match self.learn_page {
                    0 => {
                        // 技能页：点技能查看详情（描述/二形态/数值）→ 再点「购买/升级」按钮
                                ...
                                if let Some(i) = self.learn_skill_index {
                                    if let Some(&skill) = key.tree().skills_in_tree().get(i) {
                                        ...
                                        if game_core::skill::DefTable::has_alt(skill) {
                                            ...
                                            ry += 22.0;
                                            let fr = graphics::Rect::new(rx, ry, content_w, ui::theme::ROW_H);
                    ...
                    1 => {
                        ...
                        for (ni, (label, action, desc)) in rows.into_iter().enumerate().take(end).skip(start) {
                ...
                let hint = match self.learn_page {
                    ...
                    1 => "B/N/M 选分类 · 数字选中条目（[买]/[卖] 两区）· = 或回车 执行 · 滚轮/↑↓/PgUp/PgDn 滚动 · J/K/L 翻页",
...
impl event::EventHandler for Game {
    ...
    fn mouse_wheel_event(&mut self, _ctx: &mut Context, _x: f32, y: f32) -> GameResult {
        ...
        let owned_items: Vec<game_core::item::ItemId> = self
            .meta
            .profiles
            .iter()
            .find(|p| p.player_id == self.self_index())
            .map(|p| p.items.clone())
            .unwrap_or_default();
        let entries = game_core::item::shop_category_entries(self.shop_category, &owned_items);
                ...
                || ctx.keyboard.is_logical_key_just_pressed(&Key::Character("L".into())))
        {
            if let Some(lid) = self.steam_lobby_id {
                if let Some(ls) = self.steam_host_ls.as_ref() {
                    ...
                    eprintln!("[steam-room] 房间锁 -> {}", if self.steam_room_locked { "已锁定" } else { "可加入" });
                }
            }
        }
...
enum LearnAction {
    ...
    /// 选中树内第 i 个技能（点击名称=查看详情，不直接购买）
    Skill(usize),
    /// 购买/升级当前选中的技能：未购买则花钱购买（1 级），已购买则升级（等价 `=` 键）
    BuySelected,
    /// 切换技能 A/B 形态
