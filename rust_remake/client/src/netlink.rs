...
mod tests {
    ...
    fn make_cfg(slot: usize, level: u32) -> Vec<u8> {
        ...
        levels[slot] = level;
        PlayerConfig { skill_levels: levels, key_slots: [None; 8], gold: 0, gold_spent: 0, mastery: [0, 0, 0, 0], team: 0, forms: vec![false; 34], jordan_breaks: [0; 8], spell_buys: 0, items: Vec::new() }.encode()
    }
