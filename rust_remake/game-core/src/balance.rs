...
pub struct Balance {
    ...
    pub max_hp: f64,
    /// 基础生命恢复（HP/s）。**098c 实证 = 0.5/s**：
    /// 地图自维护 `In[]`（每 0.1s 由定时器 `nA` 调用 `BX(id, In[id])` 回血，`In` 初值 `.05`
    /// → 0.5/s；游戏内显示 `10*In` = "/s"；主机命令输入按每秒再 `/10` 存入）。
    /// （098b 机制文档 §3.1「每玩家 Nn=.05 生命恢复」为同一机制。）
    /// ⚠ 旧注称「098c `uhpr=0` → 无回复」有误：`uhpr` 是 WC3 单位字段（确实为 0），
    /// 而地图用自己的 `In[]` + 0.1s 定时器实现了回复。物品回复在此之上叠加。
    pub hp_regen: f64,
...
impl Balance {
    ...
    /// 098c 场地初始半径：`(9 + 人数/2) 环 × 64 码`（ta=9+qn/2；U3 修正半径口径——
    /// 2 人=640，与用户记忆「直径 20 个术士并排」一致）。全场吞没时长不变（环数×10×√人数 秒）。
