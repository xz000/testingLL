...
//! 战斗钩子 TODO（M3 2c）。
//!
//! 买价（`cost`）/卖出价（`sell`）：**已按 098c 真值校准**（2026-09-09）。
...
// 买价(cost)/卖出价(sell) 已按 098c 真值校准：买价取自 war3map_pretty.j `bD` 训练价
// （ID(id,AD,..) 的 AD，见 10579+；同家族每档同价）；卖出价取自 `ED` 回收金（10413+）。
pub const ITEMS: &[ItemDef] = &[
    // I000 速度之靴 1：+20 移速（卖 4；买 5 @bD 10627）
    ...
    // I00A 头盔 3：-32% +20 生命 -15 移速（买 9 @bD 10698；卖 24 @ED 10455）
    ...
    // I00B 坠饰 1：+10 生命（买 5 @bD 10674；卖 4 @ED 10430）
    ...
    // I00C 坠饰 3：+30 生命 +0.1 回复（买 5；卖 12 @ED 10438）
