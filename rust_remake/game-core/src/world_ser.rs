...
pub fn world_from_bytes(b: &[u8]) -> Option<World> {
    ...
    Some(World { players, arena_radius, base_regen: crate::balance::Balance::default().hp_regen, shrink_delay_secs: Fix64::from_num(crate::balance::Balance::default().shrink_ring_secs), shrink_ring_secs: Fix64::from_num(crate::balance::Balance::default().shrink_ring_secs), damage_mult: Fix64::ONE, knockback_mult: Fix64::ONE, lava_damage_mult: Fix64::ONE, sandbox, round_seed, obstacles, projectiles, eliminated_order, kills_this_round, round_number, damage_matrix, avatar_score, time, lightning_visual, mode, avatar, kings, f_override, round_forced, pending_avatar, pending_kings, shrink_timer, ice })
}
