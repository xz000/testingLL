...
fn decode_player(b: &[u8], p: &mut usize) -> Option<Player> {
    ...
    let mana = fixat(b, p)?;
    let max_mana = fixat(b, p)?;
    let mana_regen = fixat(b, p)?;
    ...
    pl.mana = mana;
    pl.max_mana = max_mana;
    pl.mana_regen = mana_regen;
...
fn encode_projectile(o: &mut Vec<u8>, pr: &Projectile) {
    ...
    match &pr.kind {
        ...
        PK::Chain { dir, speed, damage, heal, ratio, ratio_decay, life, last_target, owner, max_chain, hit_count } => { wu8(o, 9); wvec(o, *dir); wfix(o, *speed); wfix(o, *damage); wfix(o, *heal); wfix(o, *ratio); wfix(o, *ratio_decay); wfix(o, *life); wu32(o, *last_target); wu32(o, *owner); wu32(o, *max_chain); wu32(o, *hit_count); }
...
fn decode_projectile(b: &[u8], p: &mut usize) -> Option<Projectile> {
    ...
    let kind = match u8at(b, p)? {
        ...
        9 => PK::Chain { dir: vecat(b, p)?, speed: fixat(b, p)?, damage: fixat(b, p)?, heal: fixat(b, p)?, ratio: fixat(b, p)?, ratio_decay: fixat(b, p)?, life: fixat(b, p)?, last_target: u32at(b, p)?, owner: u32at(b, p)?, max_chain: u32at(b, p)?, hit_count: u32at(b, p)? },
